...
in {
      ...
      config = mkOption {
        ...
        description =
          ''An inspircd config, formatted as a nix expression.
            See the comment at the top of config.nix in the module directory for the exact semantics.
          '';
      };
  ...
  config = let
    configFile = if cfg.configFile == null then
      pkgs.writeText "inspircd.conf" (attrToConfig cfg.config)
    else
      cfg.configFile;
  in mkIf cfg.enable {
    assertions = [
    ...
    systemd.services.inspircd = {
      ...
      serviceConfig = {
        ExecStart = "${cfg.package}/bin/inspircd --nofork ${cfg.flags} ${
