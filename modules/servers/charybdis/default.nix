...
let
  ...
  # do a depends on b?
  privsetDependsOn = a: b: if a.value ? extends then a.value.extends == b.name else false;
  orderedPrivsets = (toposort (a: b: privsetDependsOn b a) (mapAttrsToList nameValuePair cfg.settings.privset)).result;
  configContents = ''
    # Generated by Nix
    /* Extensions */
